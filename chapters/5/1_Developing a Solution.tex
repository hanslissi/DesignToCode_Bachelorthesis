...
Each category has its own set of cards and "Message Templates".

\textbf{Cards}\\
Like playing cards, the cards of the DesignAPI have a front and a back. The front invites with a
title and illustration and displays its category as well as the number of cards left in that same
category. 
%%%%    % TODO: Add graphics that's like a blueprint of a card showing what's on every card (like my poster
%%%%    % P-OWL)
The back provides the user with information. It can have two to a maximum of five
different sections of content. Each section has its own purpose:
